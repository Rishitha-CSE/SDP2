...
def empregistration(request):
    ...
    if request.method == "POST":
        ...
        if formdata.is_valid():
            ...
            msg = "Failed to Register Customer! Try with valid credentials"
            return render(request, "empregistration.html", {"empform": form, "msg": msg})
...
def checkadminlogin(request):
    ...
    if flag:
        ...
        msg = "Login Failed,TRY AGAIN USER"
...
def updatedepartment(request):
    ...
    if request.method == "POST":
        ...
        loc = formdata.data['location']

        flag = Department.objects.filter(id=id)

        if flag:
            Department.objects.filter(id=id).update(name=name,location=loc)
